...
//}@dev
    ...
    if (lines[0].trim() === "#EXTM3U") {
        return isMasterFile ? _parseMasterFile(lines) // { VERSION, MASTER, LINK:[] }
                            : _parseIndexFile(lines); // { VERSION, MASTER, END, CACHE, DURATION, SEQUENCE, LINK:[] }
    }
    return null;
}
...
function _parseIndexFile(lines) { // @arg LineStringArray - ["#EXTM3U", "#EXT-X-VERSION:3", ...]
    ...
    var result = {
            "VERSION":  0,      // #EXT-X-VERSION value
            "MASTER":   false,  // has not #EXT-X-STREAM-INF
...
function M3U_build(obj) { // @ret M3UObject - master file. { VERSION, MASTER, LINK:[{ URL, CODECS, BANDWIDTH, RESOLUTION }, ...] }
                          //                  index file.  { VERSION, MASTER, END, CACHE, DURATION, SEQUENCE, LINK:[{ URL, DURATION, TITLE }, ...] }
                          // @ret M3UFormatString
//{@dev
    if (VERIFY) {
