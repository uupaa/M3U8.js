...
function M3U_parse(str) { // @arg M3UFormatString - M3U/M3U8 format string
                          // @ret M3UMasterObject|M3UIndexObject|null - M3UMasterObject = { VERSION, MASTER, STREAM }
                          //                                            M3UIndexObject  = { VERSION, INDEX, END, CACHE, DURATION, SEQUENCE, STREAM }
//{@dev
    if (VERIFY) {
...
//}@dev
    ...
    if (lines[0].trim() === "#EXTM3U") {
        return isMasterObject ? _parseMasterFile(lines) // { VERSION, MASTER, STREAM:[] }
                              : _parseIndexFile(lines); // { VERSION, INDEX, END, CACHE, DURATION, SEQUENCE, STREAM:[] }
    }
    return null;
}
...
function _parseMasterFile(lines) { // @arg LineStringArray - ["#EXTM3U", "#EXT-X-VERSION:3", ...]
    ...
    var masterObject = {
            ...
            "STREAM":   [],     // #EXTINF and URL values [{ URL, CODECS, BANDWIDTH, RESOLUTION }, ...]
        };
    var info = {};
...
function _parseIndexFile(lines) { // @arg LineStringArray - ["#EXTM3U", "#EXT-X-VERSION:3", ...]
    ...
    var indexObject = {
            ...
            "STREAM":   [],     // #EXTINF and URL values [{ URL, DURATION, TITLE, BANDWIDTH, CODECS, RESOLUTION }, ...]
        };
    var itemTitle = "";
    var itemDuration = "";
...
function M3U_build(obj) { // @ret M3UMasterObject|M3UIndexObject - M3UMasterObject = { VERSION, MASTER, STREAM:[{ URL, CODECS, BANDWIDTH, RESOLUTION }, ...] }
                          //                                       M3UIndexObject  = { VERSION, INDEX, END, CACHE, DURATION, SEQUENCE, STREAM:[{ URL, DURATION, TITLE }, ...] }
                          // @ret M3UFormatString
//{@dev
    if (VERIFY) {
