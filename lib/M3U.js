...
function M3U_parse(str) { // @arg M3UFormatString - M3U/M3U8 format string
                          // @ret M3UObject - { VERSION, MASTER, END, CACHE, DURATION, SEQUENCE, ITEMS }
//{@dev
    if (!global["BENCHMARK"]) {
...
//}@dev
    ...
    var result = {
            ...
            "MASTER":   isMasterFile, // has #EXT-X-STREAM-INF
    ...
    if (lines[0].trim() === "#EXTM3U") {
        return isMasterFile ? _parseMasterFile(lines, result)
                            : _parseIndexFile(lines, result);
    }
    return result;
}

function _parseMasterFile(lines, result) {
    var info = {};
        ...
        if (line) {
            if (line[0] === "#") {
                ...
                result["LINK"].push({
                    "URL":          line,
                    "CODECS":       info["CODECS"]     || "",
                    "BANDWIDTH":    parseFloat(info["BANDWIDTH"]) || 0,
...
function _parseIndexFile(lines, result) {
    var itemTitle = "";
    var itemDuration = 0;
