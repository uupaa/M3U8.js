...
function M3U8MovieSpooler(url,             // @arg URLString
    ...
    this._cachedDuration = 0;  // sum(ts.duration) if STATE_BLOB_CACHED
...
function _fetch() { // @desc playlist.m3u8 を読み込み ts ファイルの情報をマージする
    ...
    if (this._src) {
        // Liveはm3u8を定期的に再読み込みする必要があるがMovieは1度だけで良い
...
function M3U8MovieSpooler_seek(seekTime) { // @arg UINT32 - seek target time, seek estimated time.
...
function M3U8MovieSpooler_release(tsIndexArray) { // @arg UINT32Array - [tsIndex, ...]
    var now = Date.now();
...
function _merge(that, stream) { // @arg IndexPlaylistStreamObjectArray - [{ index, url, duration, title, fragment }, ...]
                                // @desc playlist.m3u8 の内容を tsMap にマージする。Movieでは1度しか呼ばれない
    var now = Date.now();
...
function _load(that, tsIndex, loadedCallback) {
    ...
    FileLoader["loadBlob"](tsMap[tsIndex]["url"], function(blob) {
        if (tsIndex in tsMap) {
        ...
        if (error.message === "TIMEOUT") {
            if (that._eventCallback) {
                that._eventCallback({ "type": "timeout", "detail": 408 });
            }
        } else {
            if (that._eventCallback) {
                that._eventCallback({ "type": "error", "detail": 404 });
            }
        }
