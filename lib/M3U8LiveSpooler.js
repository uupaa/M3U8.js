...
function M3U8LiveSpooler(url,       // @arg URLString
    ...
    this._errorCallback         = options["errorCallback"] || function(event) {
        ...
        console.error("M3U8LiveSpooler::event", event["type"], event["detail"]);
    };
    this._m3u8Callback = options["m3u8Callback"] || function() {
        if (M3U8LiveSpooler["VERBOSE"]) {
            console.info("M3U8LiveSpooler::fetched M3U8", event["type"], event["detail"]);
        }
    };
    this._tsCallback = options["tsCallback"] || function(url, tsIndex, blob) {
        if (M3U8LiveSpooler["VERBOSE"]) {
            console.info("M3U8LiveSpooler::fetched MPEG2-TS", url, tsIndex, blob["size"]);
        }
    };
...
M3U8LiveSpooler["prototype"] = Object.create(M3U8LiveSpooler, {
    ...
    "has":              { "value": M3U8LiveSpooler_has     }, // #has(ms:UINT32):Boolean
    "use":              { "value": M3U8LiveSpooler_use     }, // #use(ms:UINT32):ChunkObject|null
    "seek":             { "value": M3U8LiveSpooler_seek    }, // #seek(seekTime:UINT32):Object|null - { index, startTime, endTime }
    "start":            { "value": M3U8LiveSpooler_start   }, // #start():void
    "stop":             { "value": M3U8LiveSpooler_stop    }, // #stop():void
    "clear":            { "value": M3U8LiveSpooler_clear   }, // #clear():void
    "release":          { "value": M3U8LiveSpooler_release }, // #release(indexes:UINT32Array):void
...
function M3U8LiveSpooler_seek(seekTime) { // @arg UINT32 - seek target time, seek estimated time.
...
function M3U8LiveSpooler_release(tsIndexArray) { // @arg UINT32Array - [tsIndex, ...]
    var now = Date.now();
