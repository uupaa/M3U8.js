...
- IndexPlaylistObject
    - IndexPlaylistObject: { url, type, version, cache, duration, sequence, stream }
...
//}@dev
    ...
    if (lines[0].trim() === "#EXTM3U") {
        return isMasterPlaylist ? _parseMasterPlaylist(lines, playlistURL)  // { url, type, version, stream }
                                : _patseIndexPlaylist(lines,  playlistURL); // { url, type, version, end, cache, duration, sequence, stream }
    }
    return null;
}
