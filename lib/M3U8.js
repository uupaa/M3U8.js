...
var M3U8 = {
    ...
    "parse":        M3U8_parse,  // M3U8.parse(str:M3U8FormatString, url:PlaylistURLString = ""):MasterPlaylistObject|MediaPlaylistObject|null
...
//}@dev
    ...
    if (lines[0].trim() === "#EXTM3U") {
        return isMasterPlaylist ? _parseMasterPlaylist(lines, playlistURL)  // { url, type, version, streams }
                                : _parseMediaPlaylist(lines,  playlistURL); // { url, type, version, mediaSegments, ... }
    }
...
function _parseMediaPlaylist(lines,         // @arg LineStringArray - ["#EXTM3U", "#EXT-X-VERSION:3", ...]
                             playlistURL) { // @arg MediaPlaylistURLString
                                            // @ret MediaPlaylistObject
    var mediaPlaylistObject = {
            "url":              playlistURL,    // MediaPlaylistURL
        ...
        if (line) {
            if (/^#EXT/.test(line)) { // The Tag-line (^#EXT...)
                ...
                mediaPlaylistObject["mediaSegments"].push({
                    ...
                    "tsURL":        _toAbsoluteURL(line, playlistURL),
