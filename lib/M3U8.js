...
- MediaPlaylistObject: { url, type, version, allowCache, mediaSequence, mediaSegments, targetDuration, totalDurations }
    ...
    - type:             String - "VOD" or "LIVE" or "NRTLIVE"
...
function M3U8_load(url,           // @arg URLString - MasterPlaylist or MediaPlaylist
                   ...
                   options) {     // @arg Object = null - { timeout:UINT32 }
//{@dev
    if (VERIFY) {
...
function M3U8_collectPlaylist(url,           // @arg URLString - MasterPlaylist or MediaPlaylist
                              readyCallback, // @arg Function - readyCallback(playlists:PlaylistArray):void
                              errorCallback, // @arg Function = null - errorCallback(error:Error, url:URLString, code:HTTPStatusCodeUINT16):void
                              options) {     // @arg Object = null - { timeout:UINT32 }
//{@dev
    if (VERIFY) {
        $valid($type(url,           "URLString"),     M3U8_collectPlaylist, "url");
        $valid($type(readyCallback, "Function"),      M3U8_collectPlaylist, "readyCallback");
        $valid($type(errorCallback, "Function|omit"), M3U8_collectPlaylist, "errorCallback");
        ...
        if (length) {
            ...
            masterPlaylist["streams"].forEach(function(stream, index) {
                M3U8["load"](stream["url"], function(m3u8, m3u8URL) {
                    playlists.push( M3U8["parse"](m3u8, m3u8URL) ); // MediaPlaylist
                    donefn2();
                }, donefn2, options);
            });
        } else {
...
function M3U8_loadMediaPlaylist(url,                  // @arg URLString - MasterPlaylist or MediaPlaylist
...
//{@dev
    if (VERIFY) {
        ...
        $valid($type(readyCallback,        "Function"),      M3U8_loadMediaPlaylist, "readyCallback");
        $valid($type(errorCallback,        "Function|omit"), M3U8_loadMediaPlaylist, "errorCallback");
        $valid($type(streamFilterCallback, "Function|omit"), M3U8_loadMediaPlaylist, "streamFilterCallback");
...
//}@dev
    ...
    M3U8_load(url, function(m3u8, m3u8URL) {
        var playlist = M3U8_parse(m3u8, m3u8URL); // MasterPlaylistObject|MediaPlaylistObject

        switch (playlist["type"]) {
        case "MASTER":  _parseMasterPlaylist(playlist["streams"], streamFilterCallback(playlist["streams"]), m3u8URL); break;
        case "NRTLIVE":
        case "LIVE":
        case "VOD":     readyCallback(m3u8, m3u8URL, playlist);
        }
    }, errorCallback, options);

    function _parseMasterPlaylist(streams, streamIndex, m3u8URL) {
        if (streams.length && streams[streamIndex] && streams[streamIndex]["url"]) {
            M3U8_load(streams[streamIndex]["url"], function(m3u8, m3u8URL) {
                readyCallback(m3u8, m3u8URL, M3U8_parse(m3u8, m3u8URL));
            }, errorCallback, options);
        } else {
            var error = new Error("Sorry, There is no playable stream: " + _toString(streams));
...
function _toString(streams) { // @arg MasterStreamObjectArray
    var result = [];
...
function _parseMediaPlaylist(lines, // @arg LineStringArray - ["#EXTM3U", "#EXT-X-VERSION:3", ...]
    ...
    if (mediaPlaylistObject["type"] === "LIVE" &&
...
//}@dev
    ...
    if (!isMasterPlaylist)          { lines.push("#EXT-X-ALLOW-CACHE:"    + (playlist["allowCache"] ? "YES" : "NO")); }
    ...
    for (var i = 0, iz = list.length; i < iz; ++i) {
        ...
        if (isMasterPlaylist) {
            ...
            lines.push("#EXTINF:" + (p["tsDuration"] / 1000).toFixed(3) + "," + (p["tsTitle"] || ""));
            lines.push(p["tsURL"]);
        }
    }
...
function _trimMediaSegments(playlist,                // @arg MediaPlaylistObject - { url, type, version, allowCache, mediaSequence, mediaSegments, targetDuration, totalDurations }
    ...
    // skip media segments
    var i = 0, iz = mediaSegments.length;
    for (; i < iz; ++i) {
...
function M3U8_defaultFilter(masterStreams,  // @arg MasterStreamObjectArray - [MasterStreamObject, ...]
                            videoCanPlay,   // @options.videoCanPlay RegExp = /Base/ - video can play profile.
                            audioCanPlay) { // @options.audioCanPlay RegExp = /AAC/  - audio can play profile.
...
function M3U8_baselineFilter(masterStreams) { // @arg MasterStreamObjectArray - [MasterStreamObject, ...]
                                              // @ret UINT8 - stream-index or 255
