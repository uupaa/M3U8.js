...
// --- class / interfaces ----------------------------------
function M3U8Spooler(eventCallback) {
    ...
    this._indexPlaylist     = {};
    this._videoCanPlay      = /^Base/;   // video can play profile.
    this._audioCanPlay      = /AAC/;     // audio can play profile.
...
M3U8Spooler["prototype"] = Object.create(M3U8Spooler, {
    ...
    "cachedBytes":  { "get": M3U8Spooler_getCachedBytes  },
...
function _createSpoolerInstance(that, playlist, url, readyCallback, errorCallback) {
    ...
    case "MOVIE":
        ...
        break;
    default:
        errorCallback(new Error("Unknown type"));
    }
}
...
function M3U8Spooler_getCachedBytes() {
    if (this._spooler) {
        return this._spooler["cachedBytes"];
    }
    return 0;
}
