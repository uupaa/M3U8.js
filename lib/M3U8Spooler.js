...
// --- class / interfaces ----------------------------------
function M3U8Spooler(options) { // @arg Object - { durationThreshold, overthresholdCallback, errorCallback, videoCanPlay, audioCanPlay }
...
//{@dev
    if (VERIFY) {
        $valid($type(options, "Object"), M3U8Spooler, "options");
        if (options) {
...
//}@dev
    ...
    this._errorCallback         = options["errorCallback"] || function(event) { // @arg EventLikeObject - { type, detail: { url, code } }
...
function _createSpoolerInstance(that, playlist, url, readyCallback, errorCallback) {
    ...
    switch (playlist["type"]) {
    case "LIVE":
        that._spooler = new M3U8LiveSpooler(playlist["url"], that._durationThreshold,
                                            that._overthresholdCallback, that._errorCallback);
        that["start"]();
        readyCallback();
        break;
    case "VOD":
        that._spooler = new M3U8VODSpooler(playlist["url"], that._durationThreshold,
                                           that._overthresholdCallback, that._errorCallback);
        that["start"]();
        readyCallback();
        break;
