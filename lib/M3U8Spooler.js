...
var M3U8VODSpooler    = WebModule["M3U8VODSpooler"];
// --- import / local extract functions --------------------
// --- define / local variables ----------------------------
// --- class / interfaces ----------------------------------
function M3U8Spooler(options) { // @arg Object = null - { durationThreshold, overthresholdCallback, errorCallback, fetchM3U8Callback, loadTSCallback, videoCanPlay, audioCanPlay }
                                // @options.durationThreshold UINT32 = 0
                                // @options.overthresholdCallback Function = null
                                // @options.errorCallback Function = null
                                // @options.fetchM3U8Callback Function = null
                                // @options.loadTSCallback Function = null
                                // @options.videoCanPlay RegExp = /^(Base|Main)/
                                // @options.audioCanPlay RegExp = /AAC/
//{@dev
    if (VERIFY) {
        $valid($type(options, "Object|omit"),   M3U8Spooler, "options");
        if (options) {
            $valid($type(options.durationThreshold,     "UINT32|omit"),     M3U8Spooler, "options.durationThreshold");
            $valid($type(options.overthresholdCallback, "Function|omit"),   M3U8Spooler, "options.overthresholdCallback");
            $valid($type(options.errorCallback,         "Function|omit"),   M3U8Spooler, "options.errorCallback");
            $valid($type(options.fetchM3U8Callback,     "Function|omit"),   M3U8Spooler, "options.fetchM3U8Callback");
            $valid($type(options.loadTSCallback,        "Function|omit"),   M3U8Spooler, "options.loadTSCallback");
...
//}@dev
    ...
    this._src               = "";
    this._spooler           = null; // M3U8VODSpooler|M3U8LiveSpooler
    this._masterPlaylist    = {};
    this._indexPlaylist     = {};
    this._videoCanPlay      = options["videoCanPlay"] || /^(Base|Main)/;   // video can play profile.
    this._audioCanPlay      = options["audioCanPlay"] || /AAC/;            // audio can play profile.
...
function _createSpoolerInstance(that, playlist, url, readyCallback, errorCallback) {
    ...
    switch (playlist["type"]) {
    case "LIVE":
        that._spooler = new M3U8LiveSpooler(playlist["url"], that._options);
        that["start"]();
        readyCallback();
        break;
    case "VOD":
        that._spooler = new M3U8VODSpooler(playlist["url"], that._options);
        that["start"]();
        readyCallback();
        break;
    default:
